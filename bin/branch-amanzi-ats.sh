...
if [ -d amanzi/repos/dev-$1 ]; then
    echo "Repo: amanzi/dev-$1 already exists."
    exit 1;
fi
if [ -d ats/repos/dev-$1 ]; then
    echo "Repo: ats/dev-$1 already exists."
    exit 1;
fi
...
gsed -i "s/amanziname dev/amanziname dev-$1/g" modulefiles/ats/dev-$1
...
# clone the AMANZI_SRC_DIR
echo "Cloning Amanzi into new repo..."
cd amanzi/repos/
git clone dev dev-$1
...
# configure amanzi
echo "Configure/build/install Amanzi..."
. config-files/configure-amanzi.sh
cd ${ATS_SYSTEM_BASE}
